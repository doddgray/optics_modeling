import ../RII_db_tools/RII_db_tools" as rii
import numpy as np
import scipy as sp
from os import path
#u = rii.return_u()
...
import sys
sys.path.append('/home/dodd/google-drive/data/Si Ridge Waveguide Sims/Si_complex_index_model')
data_dir = '/home/dodd/google-drive/data/Si Ridge Waveguide Sims/Si_complex_index_model'
...
for Tind, TT in enumerate(T_Li):
    ...
    n_T_lm_Li[Tind,:] = n_model(lm_Li*u.um)
...
def si_complex_dispersion(lm, T, Ne, Nh):
    nr = n_r_si_lm_T(lm,T)
    ni = n_i_si_lm_T(lm,T)
    return nr - 1j*ni + si_plasma_refraction(lm,Ne,Nh)
